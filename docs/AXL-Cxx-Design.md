...
- **How `libstdc++.a` reaches an SDK consumer. CLOSED 2026-08-08 — it does
  ...
  **Superseded recommendation: keep resolving it from the installed toolchain
  and ship nothing.** `axl-c++ --hosted` already does exactly that
  (`$GXX_BIN -print-file-name=libstdc++.a`), which is why the licensing
  question has no cost today. Vendoring the two members is legally
