# AXL Roadmap — ARCHIVE (full historical detail)

> **This is the archived, full-detail phase history.** The curated
> front-page lives in [ROADMAP.md](ROADMAP.md); [CHANGELOG.md](../CHANGELOG.md)
> is authoritative for what shipped per version. This file is kept for
> reference (every completed checkbox, hardware findings, original design
> notes); it is NOT maintained line-by-line going forward — add new planning
> to ROADMAP.md or a design doc instead.
...
> **v1.2.0** — AxlCompress (gzip/zlib/DEFLATE) + AxlTar (ustar + `tar` tool)
> + AxlEdid + AxlGfx display/EDID/HiDPI accessors; HF2.3/2.4 device-manifest
> capture + HF4 SPD; `run-qemu.sh` → `axl-emulate` fixture split.
> **v1.3.1** — AxlPci descending-cap-walk fix; **absolute-pointer / touch
> seat** (`axl_input_attach_touch` + compositor seat, #GP re-enum fix);
> AxlArgs `?` help alias + terser help.
> **v1.4.0** — Regex `NOTBOL` / `NOTEOL` match flags.
> **v1.5.0** — AxlCursor absolute-pointer tracking (`axl_cursor_attach_ex` +
> `AxlCursorConfig`) + ConsoleIn-only default + AxlArgs ASCII `-` help
> separator; **v1.5.1** — console/log ASCII sweep + a `check-output-ascii`
> CI guard.
> **v1.6.0** — AxlArgs opt-in case-insensitive verb matching.
> **v1.7.0** — `axl_set_exit_status` / `axl_exit_status` (exit with a
> verbatim `EFI_STATUS`) + AxlArgs opt-in compact DOS flag syntax
> (`-x:v` / `-xv` / `/x` / `/sVar`); **v1.7.1** — the `--minimal-runtime`
> CRT0 honors the armed exit status too.
> A `scripts/cut-release.sh` automates the release cut (see docs/RELEASING.md).
> Unit tests now total **6319 both arches** (point-in-time counts quoted in
> older sections below are historical).
>
> **Active / planned sub-projects (design docs, pre-code planning):**
> [AXL-Dashboard-Server-Design.md](AXL-Dashboard-Server-Design.md) — native-SPA
> dashboard server (SSE/pubsub push, static-asset pipeline, multipart upload,
> auth/RBAC, OData helpers);
> [AXL-HII-Design.md](AXL-HII-Design.md) — headless HII forms engine `axl-hii`
> (IFR parse + expression VM + config routing; AGT renders via
> `AgtFormBrowser`);
> and the **SoftBMC EDK2 → axl-sdk port** (Phase 10 below), the flagship
> consumer migration that drives the two above.
...
- [-] Add more protocols to manifest as needed — PCI + USB **done** (AxlPci /
      AxlUsb shipped); **HII pending** (planned, see
      [AXL-HII-Design.md](AXL-HII-Design.md))
...
### Phase 10: SoftBMC — full EDK2 → axl-sdk port

The flagship consumer migration: re-base SoftBMC (the BMC firmware app)
off EDK2 onto axl-sdk + AGT **end to end** — not just the networking
stack. This is a major sub-project that *drives* much of the substrate
roadmap (the dashboard server, the HII forms engine, auth/TLS, async
firmware update), so its requirements feed back into those design docs.
Likely warrants its own design doc once scoped.

- [ ] Re-base the build off EDK2 onto the axl-sdk native toolchain (`axl-cc`,
      native CRT0, no EDK2 tree)
- [ ] Migrate to the AXL networking stack (HTTP server/client, TLS)
- [ ] Web dashboard on the native-SPA server route
      ([AXL-Dashboard-Server-Design.md](AXL-Dashboard-Server-Design.md))
- [ ] BIOS attributes / setup via the headless HII engine
      ([AXL-HII-Design.md](AXL-HII-Design.md)) + AGT `AgtFormBrowser`
- [ ] Firmware-update endpoint on AxlAsync (Phase A2) + AxlBufPool (Phase A1)
- [ ] VNC / remote console on the AxlGfx + absolute-pointer seat substrate
- [ ] Sensors / EC / SEL via AxlIpmi + AxlSmbus + AxlPubsub
