...
**Status: COMPLETE 2026-08-15** (accepted 2026-08-12, proposed 2026-08-03).
All six steps are resolved: **1-4 landed** (`41de5751`, `831e1cbf`, and §6c for
why step 4's WS queue stays), **step 5 was DROPPED** — no consumer surveyed can
use a receive queue (§6g) — and **step 6 was moot**, its watermarks having been
deleted by `23c75ee0`. The defects §6b listed as making the branch unmergeable
are fixed by §6e/§6f. Nothing here is outstanding work; what follows is the
record of how it was decided.

An earlier revision of this header read "IN PROGRESS ... Step 1 of 6 landed",
which stayed put while steps 2-4 landed and 5-6 closed. It is noted because a
status line that only ever moves forwards when someone remembers is how a
finished design reads as open for a month.
...
### 1a. The same bug was live in two more places (measured 2026-08-12) — FIXED

**Fixed by step 3 (`831e1cbf`) and shipped in v3.2.0** — the CHANGELOG entry
"~1 request in 3 was dropped under concurrent TLS handshakes" is this defect.
The `AXL_BUSY` branches it depended on are gone from `axl-tls.c` and
`axl-http-ws.c`; §6 records which step did it. The measurement below is kept
because it is what justified doing steps 2-4 before the release rather than
after.

As measured on 2026-08-12, before the fix: `0a9f81fe` fixed the `!= AXL_OK`
test in `handshake_flush_async`, the identical test survived on the response
path, and it was dropping connections:
