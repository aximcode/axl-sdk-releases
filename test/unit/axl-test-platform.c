...
/* Hand-assembled definition blocks. Written out byte by byte so the
   INPUT is part of the assertion -- a fixture generated by the code
   under test would prove nothing. Each is prefixed with a 36-byte ACPI
   header whose length field and checksum are fixed up at run time by
   aml_fixture_build.

   Layout of the interesting parts:
     ScopeOp    0x10  PkgLength  NameString  <body>
     DeviceOp   0x5B 0x82  PkgLength  NameSeg  <body>
     NameOp     0x08  NameSeg  <data>
     MethodOp   0x14  PkgLength  NameSeg  flags  <body>
     IfOp       0xA0  PkgLength  <predicate> <body>
   NameString: 0x5C root prefix, 0x2E dual prefix, 0x2F multi prefix. */

/* Scope(\_SB) { Device(DEV0) { Name(_ADR, 0x001C0004) } }

   PkgLength counts itself plus everything to the end of its package,
   and both values here are under 0x3F so each is a single byte:
     Device: 1 (PkgLength) + 4 (NameSeg) + 10 (TermList) = 15
     Scope:  1 (PkgLength) + 5 (NameString) + 17 (TermList) = 23 */
static const unsigned char aml_one_device[] = {
    0x10, 0x17,                              /* ScopeOp, PkgLength 23      */
    0x5C, '_','S','B','_',                   /* NameString \_SB_           */
    0x5B, 0x82, 0x0F,                        /* DeviceOp, PkgLength 15     */
    'D','E','V','0',                         /* NameSeg                    */
    0x08, '_','A','D','R',                   /* NameOp _ADR                */
    0x0C, 0x04, 0x00, 0x1C, 0x00             /* DWordPrefix 0x001C0004     */
};

/* Build a complete table around a definition block: 36-byte header,
   correct length, correct checksum. Returns bytes written. */
static size_t
aml_fixture_build(unsigned char *buf, size_t cap,
                  const unsigned char *body, size_t body_len)
{
    static const unsigned char hdr[36] = {
        'D','S','D','T',  0,0,0,0,  0x02, 0x00,
        'A','X','L','T','S','T',
        'F','I','X','T','U','R','E',' ',
        0x01,0,0,0,  'A','X','L',' ',  0x01,0,0,0
    };
    size_t total = sizeof(hdr) + body_len;
    if (cap < total) {
        return 0;
    }
    axl_memcpy(buf, hdr, sizeof(hdr));
    axl_memcpy(buf + sizeof(hdr), body, body_len);

    buf[4] = (unsigned char)(total & 0xFF);
    buf[5] = (unsigned char)((total >> 8) & 0xFF);
    buf[6] = (unsigned char)((total >> 16) & 0xFF);
    buf[7] = (unsigned char)((total >> 24) & 0xFF);
    ...
    buf[9] = (unsigned char)((0x100u - (sum & 0xFF)) & 0xFF);
    return total;
}

static void
test_aml_walk_basic(void)
{
    unsigned char buf[256];
    AxlAmlWalk    walk;
    AxlAmlNode    node;

    size_t n = aml_fixture_build(buf, sizeof(buf),
                                 aml_one_device, sizeof(aml_one_device));
    test_check(n == 36 + sizeof(aml_one_device),
               "aml: fixture builds to header + body");

    const AxlAcpiHeader *t = (const AxlAcpiHeader *)buf;
    test_check(axl_acpi_checksum_ok(t),
               "aml: hand-built fixture has a valid ACPI checksum");

    test_check(axl_aml_walk_begin(&walk, t) == AXL_OK,
               "aml: begin on a well-formed table");
    test_check(axl_aml_walk_next(&walk, &node),
               "aml: one device yielded");
    test_check(node.adr.kind == AXL_AML_VALUE_STATIC,
               "aml: _ADR read as a static value");
    test_check(node.adr.value == 0x001C0004u,
               "aml: _ADR carries its exact literal");
    test_check(node.sun.kind == AXL_AML_VALUE_ABSENT,
               "aml: undeclared _SUN is ABSENT, not a zero value");
    test_check(!node.conditional,
               "aml: a device outside any If is not conditional");
    test_check(!axl_aml_walk_next(&walk, &node),
               "aml: walk ends after the only device");
    test_check(!axl_aml_walk_truncated(&walk),
               "aml: clean end is not reported as truncated");
}

static void
test_aml_walk_depth_cap(void)
{
    /* Regression: the Device branch used to hit AXL_AML_DEPTH_MAX and
       `return true` WITHOUT advancing the cursor, so the walker yielded
       the same device forever and lsacpi -n hung. The depth cap exists
       to bound a hostile table and was itself the unbounded path.

       Real firmware nests 5-10 deep, so no capture reaches this; it
       needs a table built for it. The drain loop is guarded so a
       regression FAILS instead of hanging the suite -- one QEMU boot
       runs every unit binary under one timeout. */
    static unsigned char body[4096];
    size_t              n = 0;
    const unsigned      depth = AXL_AML_DEPTH_MAX + 4;

    /* depth nested Devices. Every level uses a TWO-byte PkgLength so
       the outer packages can exceed 0x3F -- with a 1-byte length the
       nesting silently collapses and the cap is never reached, which
       is exactly how the first version of this test passed against the
       bug it was written for.

       Per level: DeviceOp(2) + PkgLength(2) + NameSeg(4) = 8 bytes,
       and PkgLength counts itself plus everything to the end of its
       package, so level d spans 6 + 8*(depth-1-d). */
    for (unsigned d = 0; d < depth; d++) {
        size_t len = 6 + 8 * (size_t)(depth - 1 - d);
        body[n++] = 0x5B; body[n++] = 0x82;                  /* DeviceOp */
        body[n++] = (unsigned char)(0x40 | (len & 0x0F));    /* 1 extra byte */
        body[n++] = (unsigned char)((len >> 4) & 0xFF);
        body[n++] = (unsigned char)('A' + (d % 26));
        body[n++] = 'V'; body[n++] = '0'; body[n++] = '0';
    }

    unsigned char buf[4096 + 64];
    size_t total = aml_fixture_build(buf, sizeof(buf), body, n);
    test_check(total > 0, "aml depth: deep fixture built");

    AxlAmlWalk walk;
    AxlAmlNode node;
    test_check(axl_aml_walk_begin(&walk, (const AxlAcpiHeader *)buf) == AXL_OK,
               "aml depth: begin on a deeply nested table");

    unsigned yielded = 0;
    while (axl_aml_walk_next(&walk, &node) && yielded < 4096u) {
        yielded++;
    }
    test_check(yielded < 4096u,
               "aml depth: exceeding AXL_AML_DEPTH_MAX terminates the walk");
    test_check(axl_aml_walk_truncated(&walk),
               "aml depth: hitting the depth cap is reported as incomplete");
}

static void
test_aml_walk_bounded(void)
{
    unsigned char buf[256];
    AxlAmlWalk    walk;
    AxlAmlNode    node;

    /* A PkgLength that runs past the end of the table must stop the
       walk rather than read past it. Firmware is untrusted input, so
       this is the normal case, not an edge case. */
    size_t n = aml_fixture_build(buf, sizeof(buf),
                                 aml_one_device, sizeof(aml_one_device));
    test_check(n > 37, "aml: fixture large enough to corrupt");
    /* Offset 36 is the ScopeOp, 37 its PkgLength. 0x3F stays a
       single-byte encoding (bits 7:6 clear) but claims 63 bytes where
       only 22 remain -- an overrun, not a re-encoding. */
    buf[37] = 0x3F;

    test_check(axl_aml_walk_begin(&walk, (const AxlAcpiHeader *)buf) == AXL_OK,
               "aml: begin succeeds on a table with a bad PkgLength");
    unsigned guard = 0;
    while (axl_aml_walk_next(&walk, &node) && guard < 1000u) {
        guard++;
    }
    test_check(guard < 1000u,
               "aml: an overrunning PkgLength does not loop forever");
    test_check(axl_aml_walk_truncated(&walk),
               "aml: an overrunning PkgLength is reported as truncated");

    /* A table whose length field is smaller than a header cannot hold
       a definition block at all. */
    unsigned char stub[36];
    axl_memcpy(stub, buf, sizeof(stub));
    stub[4] = 0x10; stub[5] = 0; stub[6] = 0; stub[7] = 0;   /* length 16 */
    test_check(axl_aml_walk_begin(&walk, (const AxlAcpiHeader *)stub) == AXL_ERR,
               "aml: a table too short for a definition block is rejected");

    test_check(axl_aml_walk_begin(NULL, (const AxlAcpiHeader *)buf) == AXL_ERR,
               "aml: NULL walk state rejected");
    test_check(axl_aml_walk_begin(&walk, NULL) == AXL_ERR,
               "aml: NULL table rejected");
}

// ---------------------------------------------------------------------------
// Real-firmware ACPI fixtures (staged by the integration runner)
// ---------------------------------------------------------------------------

/* The captures are LOCAL-ONLY by policy -- .gitignore keeps hardware
   captures out of the repo -- so CI and fresh clones have neither and
   every fixture-backed test must SKIP with a balanced count. See
   test/fixtures/real-hw/README.md.

   Returns a malloc'd copy of the whole table, or NULL when the fixture
   was not staged. Caller frees. */
static void *
acpi_fixture_load(const char *name, size_t *out_len)
{
    char *path = axl_resolve_data_file(NULL, name);
    if (path == NULL) {
        return NULL;
    }

    AxlFileView *v = axl_file_view_open(path, 0, 8);
    axl_free(path);
    if (v == NULL) {
        return NULL;
    }

    size_t len = axl_file_view_size(v);
    void  *buf = (len > 0) ? axl_malloc(len) : NULL;
    if (buf == NULL) {
        axl_file_view_close(v);
        return NULL;
    }

    size_t got = axl_file_view_read(v, 0, buf, len);
    axl_file_view_close(v);
    if (got != len) {
        axl_free(buf);
        return NULL;
...
static void
test_acpi_fixture_staging(void)
{
    /* Infra check, deliberately ahead of the AML walker: prove a
       real-firmware table can be staged into the image and read back
       intact, so a later census failure means the WALKER is wrong
       rather than the fixture never having arrived.

       Exact sizes, not "> 0" -- these are the captured tables and a
       truncated read must fail rather than look plausible. */
    static const struct {
        const char *file;
        size_t      len;
        const char *oem;      /* OEM ID, 6 bytes at offset 10 */
    } fixtures[] = {
        { "fixture-client-dsdt.dat",  446170, "ALASKA" },
        { "fixture-server-dsdt.dat", 2137364, "DELL  " },
    };

    for (size_t i = 0; i < sizeof(fixtures) / sizeof(fixtures[0]); i++) {
        size_t len = 0;
        void  *buf = acpi_fixture_load(fixtures[i].file, &len);
        if (buf == NULL) {
            test_skip_n(3, "acpi fixture: capture not staged (local-only by policy)");
            continue;
        }

        const AxlAcpiHeader *h = (const AxlAcpiHeader *)buf;
        test_check(len == fixtures[i].len,
                   "acpi fixture: staged table has its captured byte length");
        test_check(h->signature[0] == 'D' && h->signature[1] == 'S'
                   && h->signature[2] == 'D' && h->signature[3] == 'T',
                   "acpi fixture: staged table is a DSDT");
        test_check(axl_memcmp(h->oem_id, fixtures[i].oem, 6) == 0,
                   "acpi fixture: staged table carries its captured OEM ID");
        axl_free(buf);
    }
}

/* Census of one staged table, for grading the walker against real
   firmware. Counts are per-table and compared with iasl's, which is an
   independent implementation of the same parse. */
typedef struct {
    size_t devices;
    size_t adr_static, adr_method;
    size_t sun_static, sun_method;
    size_t uid_static, uid_method, uid_nonint;
    size_t seg_static, seg_method;
    size_t bbn_static, bbn_method;
    size_t pld_static, pld_method;
    size_t conditional;
    bool   truncated;
} AmlCensus;

static void
aml_census_count(AxlAmlValueKind k, size_t *st, size_t *me)
{
    if (k == AXL_AML_VALUE_STATIC) { (*st)++; }
    else if (k == AXL_AML_VALUE_METHOD) { (*me)++; }
}

static bool
aml_census(const char *file, AmlCensus *c)
{
    size_t len = 0;
    void  *buf = acpi_fixture_load(file, &len);
    if (buf == NULL) {
        return false;
    }
    ...
    AxlAmlWalk walk;
    AxlAmlNode n;
    if (axl_aml_walk_begin(&walk, (const AxlAcpiHeader *)buf) != AXL_OK) {
        axl_free(buf);
        return false;
    }
    while (axl_aml_walk_next(&walk, &n)) {
        c->devices++;
        if (n.conditional) { c->conditional++; }
        aml_census_count(n.adr.kind, &c->adr_static, &c->adr_method);
        aml_census_count(n.sun.kind, &c->sun_static, &c->sun_method);
        aml_census_count(n.uid.kind, &c->uid_static, &c->uid_method);
        if (n.uid.kind == AXL_AML_VALUE_NON_INTEGER) { c->uid_nonint++; }
        aml_census_count(n.seg.kind, &c->seg_static, &c->seg_method);
        aml_census_count(n.bbn.kind, &c->bbn_static, &c->bbn_method);
        aml_census_count(n.pld_kind, &c->pld_static, &c->pld_method);
    }
    c->truncated = axl_aml_walk_truncated(&walk);
    axl_free(buf);
    return true;
}

static void
test_aml_walk_real_firmware(void)
{
    /* Grading against real firmware, which is the only thing that can
       show whether the walker survives what firmware actually emits.
       The synthetic fixtures above are 24 bytes; these are 446 KB and
       2.1 MB with 396 and 1404 devices.

       Counts are per-table and come from `iasl -d` on the same file --
       an independent implementation, the same discipline the crypto
       vectors get. Where a count differs, determine which is right
       before changing either number. */

    /* Expected counts come from an oracle that walks `iasl -d` output
       and counts objects whose IMMEDIATE enclosing block is a Device --
       the same thing the walker reports. A raw grep of the disassembly
       counts them ANYWHERE, including inside Processor and ThermalZone
       blocks and Method bodies, and disagreeing with that number is
       correct behaviour. Getting this wrong sent me chasing a
       nonexistent "missing _SUN" for a while. */

    /* Server SSDT5 also serves as the CONTROL for every sun_static == 0
       below: it publishes 25 of them, so a walker that never looked at
       _SUN would fail here rather than scoring a misleading zero on
       the machines that publish none. */
    AmlCensus ss;
    if (aml_census("fixture-server-ssdt5.dat", &ss)) {
        test_check(ss.devices == 49,
                   "aml real: server SSDT5 yields its 49 devices");
        test_check(ss.sun_static == 25,
                   "aml real: server SSDT5 yields its 25 static _SUN (walker CAN see _SUN)");
        test_check(ss.adr_static == 49,
                   "aml real: server SSDT5 yields its 49 static _ADR");
        test_check(!ss.truncated,
                   "aml real: server SSDT5 walks the whole table");
    } else {
        test_skip_n(4, "aml real: server SSDT5 not staged (local-only by policy)");
    }

    /* Client DSDT: the only fixture with Method(_ADR) and If-wrapped
       devices, which is the half a VM capture cannot exercise. */
    AmlCensus cl;
    if (aml_census("fixture-client-dsdt.dat", &cl)) {
        test_check(cl.devices == 396,
                   "aml real: client DSDT yields its 396 devices");
        test_check(cl.adr_static == 120,
                   "aml real: client DSDT yields its 120 static _ADR");
        test_check(cl.adr_method == 75,
                   "aml real: client DSDT yields its 75 Method(_ADR)");
        test_check(cl.sun_static == 0 && cl.sun_method == 0,
                   "aml real: client DSDT publishes no _SUN in either form");
        test_check(cl.bbn_method == 2,
                   "aml real: client DSDT yields its 2 Method(_BBN)");
        test_check(cl.seg_static == 1,
                   "aml real: client DSDT yields its 1 static _SEG");
        /* 155 _UID declarations, 34 of them strings. Splitting them is
           the point of AXL_AML_VALUE_NON_INTEGER: a string _UID is
           present and perfectly readable, just not a number. */
        test_check(cl.uid_static == 121,
                   "aml real: client DSDT yields its 121 integer _UID");
        test_check(cl.uid_nonint == 34,
                   "aml real: client DSDT reports its 34 string _UID as NON_INTEGER");
        test_check(cl.conditional > 0,
                   "aml real: client DSDT has devices declared inside If");
        test_check(!cl.truncated,
                   "aml real: client DSDT walks the whole table with nothing skipped");
    } else {
        test_skip_n(10, "aml real: client DSDT not staged (local-only by policy)");
    }

    /* Server DSDT: 2.1 MB, and the near-opposite profile -- every _ADR
       static, every _SEG and _BBN a Method across 17 root bridges. */
    AmlCensus sv;
    if (aml_census("fixture-server-dsdt.dat", &sv)) {
        test_check(sv.devices == 1404,
                   "aml real: server DSDT yields its 1404 devices");
        test_check(sv.adr_static == 357,
                   "aml real: server DSDT yields its 357 static _ADR");
        test_check(sv.adr_method == 0,
                   "aml real: server DSDT has no Method(_ADR)");
        test_check(sv.seg_method == 17,
                   "aml real: server DSDT yields its 17 Method(_SEG)");
        test_check(sv.bbn_method == 17,
                   "aml real: server DSDT yields its 17 Method(_BBN)");
        test_check(sv.uid_static == 1055,
                   "aml real: server DSDT yields its 1055 integer _UID");
        test_check(sv.uid_nonint == 2,
                   "aml real: server DSDT reports its 2 string _UID as NON_INTEGER");
        test_check(!sv.truncated,
                   "aml real: server DSDT walks the whole 2.1 MB table");
    } else {
        test_skip_n(8, "aml real: server DSDT not staged (local-only by policy)");
    }
}

// ---------------------------------------------------------------------------
// axl_pci_read_slot_caps — PCIe Slot Capabilities / Slot Status
// ---------------------------------------------------------------------------
...
{
    AxlPciSlotCaps caps;
    AxlPciAddr     root = { 0, 0, 0, 0 };

    /* NULL out is rejected before any config-space traffic. */
    test_check(axl_pci_read_slot_caps(root, NULL) == AXL_ERR,
               "slot caps: NULL out rejected");

    /* A function that does not respond at all. Bus 0xFF dev 0x1F func 7
       is absent on both q35 and aa64 virt; ECAM reads return all-1s and
       the capability walk's vendor-ID precheck must terminate it. */
    AxlPciAddr absent = { 0, 0xFF, 0x1F, 7 };
    caps.physical_slot_number = 0xBEEF;
    test_check(axl_pci_read_slot_caps(absent, &caps) == AXL_ERR,
               "slot caps: absent function rejected");
    test_check(caps.physical_slot_number == 0xBEEF,
               "slot caps: failure leaves out untouched");

    /* An endpoint is never a slot, however many capabilities it has.
       The virtio-rng behind the runner's root port is a real endpoint
       WITH a PCI-Express capability, which makes it the discriminating
       case: a reader that finds cap 0x10 and stops checking the
       Device/Port Type would wrongly accept it. */
    AxlPciAddr *it = NULL;
    AxlPciAddr  ep = {0};
    bool        have_ep = false;
    while ((it = axl_pci_next(it)) != NULL) {
    ...
    if (have_ep) {
        caps.physical_slot_number = 0xBEEF;
        test_check(axl_pci_read_slot_caps(ep, &caps) == AXL_ERR,
                   "slot caps: virtio endpoint with a PCIe cap is not a slot");
        test_check(caps.physical_slot_number == 0xBEEF,
                   "slot caps: endpoint rejection leaves out untouched");
    } else {
        test_skip_n(2, "slot caps: no virtio endpoint in topology");
    }
}

static void
test_pci_slot_caps_positive(void)
{
    /* The discriminating test. Every assertion in
       test_pci_slot_caps_negative also passes against a reader that
       ALWAYS returns AXL_ERR -- a cap walk that never matches, a
       port-type check that rejects everything. Only a success proves
       the reader can actually find and decode the block.

       The QEMU runner adds `-device pcie-root-port,id=axl_rp0,
       bus=pcie.0,chassis=1` on both q35 and aa64 virt
       (test/integration/common-test.sh), so a root port with Slot
       Implemented is present. Located by class 0x060400 rather than a
       hard-coded BDF, matching test_pci_cap_chain_nonmonotonic. */
    AxlPciAddr rp;
    if (axl_pci_find_by_class(0x060400, 0, &rp) != AXL_OK) {
        test_skip_n(9, "slot caps: no pcie-root-port in topology");
        return;
    }

    /* QEMU's pcie-root-port models a slot, so a failure here is a real
       defect rather than a topology gap. Asserted unconditionally so
       the populated and SKIP paths both declare 4
       (feedback_balancer_count), and so a future QEMU that stopped
       modelling the slot would fail loudly instead of silently
       reducing coverage. */
    AxlPciSlotCaps caps;
    int rc = axl_pci_read_slot_caps(rp, &caps);
    test_check(rc == AXL_OK,
               "slot caps: root port with Slot Implemented is readable");
    if (rc != AXL_OK) {
        /* caps is untouched on failure, so the field checks below
           would be reading uninitialised stack. */
        test_skip_n(8, "slot caps: root port unreadable, field checks moot");
        return;
    }

    test_check(caps.port_type == AXL_PCI_PORT_TYPE_ROOT_PORT,
               "slot caps: root port reports Device/Port Type 0x4");

    /* Presence Detect pins an exact BIT POSITION, which the checks
       above do not. The runner puts a virtio-rng BEHIND this root
       port (`-device virtio-rng-pci,bus=axl_rp0`), so the slot is
       occupied and the bit must read set. This is the assertion that
       catches the defect the contract review found: an earlier draft
       read Presence Detect from bit 5, which is MRL Sensor State and
       reads 0 on this port. */
    /* Slot CAPABILITIES bits, pinned against what QEMU actually
       reports for this port (measured: slotcap=0x0002007b). Bits 5
       and 6 are ADJACENT and both set, which is what makes this an
       off-by-one guard rather than a smoke test -- a shift that
       slipped by one would break at least one of them. The contract
       review already caught exactly that class of defect in the Slot
       Status register. */
    test_check(caps.hotplug_capable,
               "slot caps: QEMU root port reports hot-plug capable (Slot Cap bit 6)");
    test_check(caps.hotplug_surprise,
               "slot caps: QEMU root port reports hot-plug surprise (Slot Cap bit 5)");
    test_check(!caps.mrl_sensor,
               "slot caps: QEMU root port reports no MRL sensor (Slot Cap bit 2)");

    /* The runner leaves QEMU's `slot` property unset, so the Physical
       Slot Number is 0 here. */
    test_check(caps.physical_slot_number == 0,
               "slot caps: physical slot number is 0 when QEMU slot= is unset");

    /* Slot STATUS bit positions cannot be verified here at all: QEMU
       reports the whole register as 0x0000 for this port even with a
       device behind it (measured -- bridge secondary bus 01 is
       populated, yet Presence Detect reads clear). Every bit being
       zero means the data cannot discriminate bit 5 from bit 6, so an
       assertion on it would pass against a wrong shift. The bit
       positions rest on the real-hardware capture recorded in
       src/pci/axl-pci-slot.c, and this is a genuine
       real-hardware-only path. */
    test_skip_n(1, "slot caps: Slot Status bit positions (QEMU reports 0x0000)");

    test_check(caps.power_limit_scale <= 3,
               "slot caps: power limit scale is within its 2-bit field");

    /* Physical Slot Number is 13 bits (Slot Cap 31:19). The runner
       leaves QEMU's `slot` property unset, so the value itself is 0
       here -- assert the field width instead of a value the harness
       does not pin. */
    test_check(caps.physical_slot_number <= 0x1FFF,
               "slot caps: physical slot number is within its 13-bit field");
}
...
test_platform_main(int argc, char **argv)
{
    ...
    test_aml_walk_basic();
    test_aml_walk_bounded();
    test_aml_walk_depth_cap();
    test_acpi_fixture_staging();
    test_aml_walk_real_firmware();
    test_pci_slot_caps_negative();
    test_pci_slot_caps_positive();
