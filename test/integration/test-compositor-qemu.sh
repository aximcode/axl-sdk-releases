#!/bin/bash
# test-compositor-qemu.sh — AxlCompositor end-to-end present test.
#
...
# GOP, then runs compositor-selftest.efi, which presents a known scene,
...
#
# Usage: ./test/integration/test-compositor-qemu.sh [X64|AARCH64|both]
#        (default: both)
...
run_one() {
    ...
    efi="$out/compositor-selftest.efi"

    echo "=== AxlCompositor end-to-end present ($arch) ==="
    make -C "$PROJECT_DIR" ARCH="$native_arch" compositor-selftest 2>&1 | tail -2
    ...
    timeout 120s "$PROJECT_DIR/scripts/run-qemu.sh" \
        ...
        | grep -iE "Display:|PASS:|FAIL:|COMPOSITOR-SELFTEST|too small" || true

    local verdict
    verdict="$(grep -E "^COMPOSITOR-SELFTEST: [0-9]+ passed, [0-9]+ failed" "$log" | tail -1 || true)"
    rm -f "$log"
    ...
    if [[ "$verdict" =~ ,\ 0\ failed$ ]] && [[ ! "$verdict" =~ ^COMPOSITOR-SELFTEST:\ 0\ passed ]]; then
...
if (( overall_fail > 0 )); then
    echo "$overall_fail arch(es) failed the compositor present test"
    exit 1
fi
echo "All compositor present checks passed."
exit 0
