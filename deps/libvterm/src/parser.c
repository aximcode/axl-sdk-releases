...
{
  size_t pos = 0;
  /* AXL patch [5/8]: initialize. The switch below assigns in every arm of
   * enum VTermParserState, but has no `default:`, so GCC cannot prove
   * exhaustiveness and warns under -Wmaybe-uninitialized. NULL is the
   * "not inside a string state" value the trailing `if(string_start)` wants. */
...
#define IS_STRING_STATE()      (vt->parser.state >= OSC_COMMAND)
  ...
  for( ; pos < len; pos++) {
    ...
    case CSI_ARGS:
      ...
      if(c >= '0' && c <= '9') {
        /* AXL patch [7/8]: ignore digits once all CSI_ARGS_MAX slots are full.
         * argi saturates at CSI_ARGS_MAX (see the ';' and final-byte guards
         * below); this branch must not write args[CSI_ARGS_MAX], one past the
         * end of the array. args[] is the LAST field of the csi union, so that
         * write would land on the adjacent parser.callbacks pointer -- an
         * out-of-bounds write driven by untrusted input (a serial / SOL byte
         * stream). Dropping the extra digits also keeps the first 16 parameters
         * intact instead of corrupting arg 16 with arg 17's value. */
      ...
      if(c == ':') {
        /* AXL patch [7/8]: same bound as the digit branch -- do not tag a slot
         * we never allocated. */
        if(vt->parser.v.csi.argi < CSI_ARGS_MAX)
      ...
      if(c == ';') {
        /* AXL patch [7/8]: bound argi. Upstream increments unconditionally, so a
         * CSI with more than CSI_ARGS_MAX arguments walks args[] off its end onto
         * the adjacent parser.callbacks pointer -- an OOB write from untrusted
         * input. Advance up to CSI_ARGS_MAX and no further; once argi ==
         * CSI_ARGS_MAX it stays there (the digit branch above then drops further
         * args), and args[argi] is reset only while argi is still in bounds so
         * arg 16 is preserved. Net effect matches the DEC / xterm / libtsm rule:
         * process the first 16 parameters, silently drop the rest. Worth
         * reporting upstream -- the neovim fork lacks the bound; Vim's fork added
         * one in patch 9.2.0279. */
        if(vt->parser.v.csi.argi < CSI_ARGS_MAX)
          vt->parser.v.csi.argi++;
        if(vt->parser.v.csi.argi < CSI_ARGS_MAX)
      ...
      /* else fallthrough */
      /* AXL patch [7/8]: this argi++ converts the 0-based index into the count
       * do_csi consumes, so it too must saturate at CSI_ARGS_MAX -- otherwise a
       * full arg list makes do_csi read args[CSI_ARGS_MAX] out of bounds. */
      if(vt->parser.v.csi.argi < CSI_ARGS_MAX)
        vt->parser.v.csi.argi++;
      vt->parser.intermedlen = 0;
