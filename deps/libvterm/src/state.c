...
#define INTERMED(i,b) ((i << 16) | b)
  ...
  case 0x73: // CSI s
    /* AXL patch [8/8]: restore SCOSC (ANSI.SYS save cursor). This is the only
     * AXL patch that changes libvterm BEHAVIOUR rather than freestanding
     * portability or memory safety, so it raises re-vendor cost -- it must be
     * re-applied by hand, not just diffed for. xterm binds CSI s to SCOSC when
     * left/right-margin mode is off and to DECSLRM when it is on; upstream
     * binds it unconditionally to DECSLRM, which loses the SCOSC/SCORC pair
     * (widely emitted by shell prompts and TUIs) entirely. Match xterm: when
     * the margin mode is off, save the cursor via the same savecursor() that
     * backs DECSC/DECRC; otherwise fall through to the DECSLRM behavior. */
    if(!state->mode.leftrightmargin) {
      savecursor(state, 1);
      break;
    }
  ...
  case 0x75: // CSI u - SCORC (ANSI.SYS restore cursor)
    /* AXL patch [8/8]: upstream has no CSI u handler, so the SCORC restore is
     * dropped. Restore via the same savecursor() that backs DECRC. */
    savecursor(state, 0);
    break;
...
static void request_status_string(VTermState *state, VTermStringFragment frag)
{
  ...
  switch(tmp[0] | tmp[1]<<8 | tmp[2]<<16) {
    ...
    case ' '|('q'<<8): {
      // Query DECSCUSR
      /* AXL patch [6/8]: seed the reply with "steady block".
       * vterm_state_new() never assigns mode.cursor_shape, so a state queried
       * with DECRQSS-of-DECSCUSR *before* vterm_state_reset() runs would read an
       * uninitialized field, then decrement and transmit it. NOTE: this is NOT
       * reachable in AXL's usage -- axl_vterm_new() calls
       * vterm_state_reset(state, 1) right after vterm_obtain_state(), and reset
       * seeds cursor_shape to VTERM_PROP_CURSORSHAPE_BLOCK, so any query from the
       * byte stream sees an initialized value. This seed is defensive hardening
       * for the API-misuse path (obtain-then-query with no reset), not a fix for
       * a stream-reachable leak. 2 == steady block, the terminal default; blink
       * makes it 1. */
      int reply = 2;
