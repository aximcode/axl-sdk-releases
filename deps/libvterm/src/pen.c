...
INTERNAL void vterm_state_setpen(VTermState *state, const long args[], int argcount)
{
  ...
  while(argi < argcount) {
    ...
    case 4: // Underline
      ...
      /* AXL patch [7/8]: the sub-parameter is the NEXT arg slot; only consume it
       * when that slot is within argcount. A MORE flag on the last in-count arg
       * would otherwise advance argi to argcount and read args[argcount] -- and
       * with a full 16-arg list that is args[CSI_ARGS_MAX], one past the array,
       * landing on the adjacent parser.callbacks pointer. OOB read from untrusted
       * input. */
      if(argi + 1 < argcount && CSI_ARG_HAS_MORE(args[argi])) {
        argi++;
    ...
    case 38: // Foreground colour alternative palette
      /* AXL patch [7/8]: reads args[argi+1] (the colour-space selector), so that
       * slot must exist -- require argcount - argi >= 2, else a `38` in the last
       * arg reads args[argcount] (== args[16] onto callbacks at a full arg list)
       * out of bounds. lookup_colour bounds its own reads by the count passed. */
      if(argcount - argi < 2)
        return;
    ...
    case 48: // Background colour alternative palette
      /* AXL patch [7/8]: see case 38 -- args[argi+1] must be in bounds. */
      if(argcount - argi < 2)
        return;
    ...
    /* AXL patch [7/8]: bound the sub-parameter skip by argcount. A MORE flag on
     * the final arg must not walk argi past the array onto args[CSI_ARGS_MAX].
     * Short-circuit leaves argi at argcount, which the outer loop then exits. */
