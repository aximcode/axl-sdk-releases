...
/* AXL patch [2/8]: <stdio.h> and <stdlib.h> dropped. They were needed only by
 * default_malloc/default_free and vterm_check_version(), both rewritten below.
 * Dropping them keeps `exit`/`fprintf`/`stderr`/`malloc`/`free` out of the
 * link, none of which a freestanding UEFI image provides. */
#include <stdarg.h>
#include <string.h>
...
/* AXL patch [2/8]: route the default allocator through AXL's heap, so plain
 * vterm_new() works. axl_calloc(1, size) matches upstream's malloc+memset(0). */
...
{
  /* AXL patch [3/8]: AXL binds Layer 2, so screen.c is vendored but never
   * compiled -- a consumer that already owns a cell grid does not want a
   * second one. Upstream calls vterm_screen_free() here; that reference alone
   * would drag all of screen.c into every image that merely frees a VTerm.
   * vt->screen is only ever set by vterm_obtain_screen(), which also lives in
   * screen.c, so with Layer 3 out of the link it is provably NULL. Assert the
   * invariant instead of calling into the layer we did not build. */
  AXL_DEBUG_ASSERT(vt->screen == NULL);
...
/* AXL patch [4/8]: upstream reports a version mismatch with fprintf(stderr) and
 * exit(1), neither of which exists in a freestanding UEFI image. libvterm is
 * built from source into libaxl.a, so a caller's headers and the library are
 * the same tree by construction and can never disagree at runtime. Downgrade
 * to a debug assertion; the symbol stays so VTERM_CHECK_VERSION still links. */
void vterm_check_version(int major, int minor)
{
  AXL_DEBUG_ASSERT(major == VTERM_VERSION_MAJOR);
  AXL_DEBUG_ASSERT(minor <= VTERM_VERSION_MINOR);
