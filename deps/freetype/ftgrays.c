...
#define SCALED( x )  ( (x) * ( 1L << shift ) - delta )
                      ...
                      const FT_Raster_Params*  params )
  {
    ...
    /* AXL fix: upstream FreeType typo (present in current ftgrays
     * master) — the first term read `max_ex - min_ey`, mixing the
     * x-span's end with the y-span's start. When a clip box has
     * max_ex < min_ey (e.g. a glyph rotated so its narrow x-extent
     * sits at a large y-offset), this signed TCoord span goes negative
     * and converts to a huge `estimate` (the `* 10UL` promotes to
     * unsigned long); the later `(size_t)estimate * sizeof(TCell)` then
     * wraps to a small allocation while `cell_null = buffer + estimate`
     * points far past it, corrupting memory (observed as a hang
     * rasterizing rotated text via axl_ttf_draw_affine). Corrected to
     * `max_ex - min_ex` (the x-span), matching the `max_ey - min_ey`
     * y-span term. Safe: `estimate` is only a sizing hint —
