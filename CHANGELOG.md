...
> Your **code** is unaffected — `axl.h` and the ABI are unchanged, and anything
> that compiled and linked against 4.4.0 still does. What breaks is anything
> that **fetches or installs** the SDK: asset names, and the packages. See
> `docs/RELEASING.md` §"Two kinds of breaking change".


- **The `.deb` and `.rpm` are no longer published.** `install.sh` replaces
  them; see the install section of `README.md`. A consumer that runs
  `apt install ./axl-sdk.deb` or `dnf install ./axl-sdk.rpm` must move to it.
  The one capability genuinely lost is a weak dependency on the QEMU stack,
  which the installer replaces by probing and printing the distro command.
  If a policy requires a signed package, ask — `scripts/build-packages.sh`
  still builds one.

- **Every release asset is renamed and versioned.** One format string now
  names all of them, `axl-sdk-<component>-<ver>[-<arch>].tar.gz`:

  | was | is |
  |---|---|
  | `axl-sdk-<ver>-linux-x86_64.tar.gz` | `axl-sdk-linux-<ver>-x86_64.tar.gz` |
  | `axl-sdk-host-tools.tar.gz` | `axl-sdk-host-tools-<ver>.tar.gz` |
  | `axl-sdk-tools-<arch>.tar.gz` | `axl-sdk-uefi-tools-<ver>-<arch>.tar.gz` |

  **Anything pinning a filename, or fetching through
  `releases/latest/download/<asset>`, breaks.** Only `install.sh`, `VERSION`
  and `SHA256SUMS` keep stable names — they are the only ones fetched by a
  reader who does not yet know the version, and `install.sh` resolves the rest.
  `axl-sdk-tools` → `axl-sdk-uefi-tools` is the load-bearing rename: it is the
  one asset whose contents run on the machine *under test*, not the one that
  downloaded them.

- **`axl-sdk-host-tools` and `axl-sdk-uefi-tools` are no longer tarbombs.**
  They unpacked 6 and 43 entries respectively into the caller's working
  directory; each now extracts to a single versioned root matching its own
  name. **A script that extracted them with `tar xf … -C <dir>` finds its
  files one directory deeper.** `install.sh` handles both shapes, including on
  releases published before this one.

### Added

- **`lsacpi` now shows who GENERATED each table, and `-v` gives the
  dmesg-equivalent row.** Asked why a 2-socket Grace server publishes 19
  SSDTs, the inventory could not answer: signature, length, revision and OEM
  strings leave 19 near-identical rows. The **creator ID** splits them
  instantly — six carry `INTL` (hand-written ASL through Intel's iASL) and
  thirteen carry `ARMH` (generated by firmware at boot, which is why they
  arrive in repeated sizes). That column is now in the **default** row,
  because it is the field people actually arrive needing.

  `-v` adds the physical **address**, OEM revision and creator revision —
  matching what Linux prints at boot. The address is what proves N tables
  sharing a length are N *distinct* tables rather than one counted N times.
  It sits behind `-v` purely on width: the default row is 50 columns and
  fits any console, while the full row passes 80 and a UEFI console wraps.

  `--json` carries all of them plus the address unconditionally — no column
  budget there, and a consumer diffing two machines wants every header field.
  Every field was already parsed into `AxlAcpiHeader`; only the printing was
  missing. A table whose creator field is zeroed (QEMU's BGRT) renders `-`
  rather than a blank column, the same way the FACS row reports its absent
  fields.

- **`--json` now works in every `lsacpi` render mode.** It was honoured by
  the table inventory alone; `lsacpi MCFG --json`, `lsacpi -n --json` and
  `lsacpi -s --json` parsed the flag, dropped it, printed text and exited 0.
  A single table now emits the same header schema the inventory row does,
  plus its typed decode (`mcfg.windows`) or, where there is no typed reader,
  its `bytes` as hex — the JSON counterpart of the automatic hexdump. The
  namespace walk emits `devices[]`, each `_ADR`/`_SUN`/`_UID`/`_SEG`/`_BBN`/
  `_PLD` as `{kind, value}`, because "declared as a Method and unreadable
  without executing AML" is not the same fact as "not declared at all" and a
  bare number would conflate them. The slot correlation emits `slots[]` with
  its disagreements as machine-readable **codes** rather than bracketed
  English a script would have to grep; text and JSON now read one shared
  `slot_disagreements()`, so the summary count and the per-row list cannot
  drift apart.

- **Every AXL command answers `--version` now, in one shape.** All 37 target
  tools already did, through `AXL_TOOL_MAIN` -> `axl_version_handle()`. The
  host side was 3 of 11: `axl-cc` and `axl-c++` answered, `axl` answered with
  a bare unnamed version string, and the other eight — `run-qemu`,
  `profile-qemu`, `prune`, `emulate`, `rsod-decode`, `gdb-syms`,
  `extract-fv-shell`, `axl-install-toolchain` — exited 1 or 2 with "unknown
  argument". They all print `<prog> <version>` now, matching the target tools,
  via `axl_handle_version()` in `axl-common.sh` and `axl_version.py` for the
  argparse ones.

  A checkout answers from its own `VERSION`; a staged prefix from
  `share/axl/version` beside it — never the reverse, which is the wrong answer
  `axl-cc` was fixed for and still refuses outright. `axl --print-version`
  stays a bare value: it is a machine interface like `--print-prefix`, and
  only `--version` gained the program name.

  `make check-tool-version` guards it, and RUNS each command rather than
  grepping for the flag: a grep over these scripts returns three false
  positives (a comment, a line of prose, a suggestion inside an error
  message), and it reported two scripts as supporting a flag neither had. It
  checks the staged layout too, which caught a real break the source-only half
  could not see — `install-toolchain.sh` stages to `bin/` rather than
  `libexec/axl/`, so its `axl-common.sh` neighbour is one directory over.

- **`lsacpi <SIG>` decodes EVERY table with that signature, and `--at` names
  one by address.** It decoded the first match and said nothing about the rest.
  That is a silent wrong answer on the machine the view exists for: a 2-socket
  Grace server publishes 19 SSDTs, and checking its own `dmesg` inventory,
  **ten of them fall into groups identical on every ACPI header field** — one
  4-way tie and three 2-way ties across length, revision, OEM ID, OEM table ID,
  OEM revision, creator ID and creator revision. No header field can name the
  one you meant; only the physical address can, which is what `--at` takes.
  `--json` now emits the inventory's envelope from every view — `{"tables":
  [...], "count": N}` — so one signature matching nineteen tables is not a
  different schema from one matching one.

- **The default `lsacpi` row carries every header field.** Address, length,
  revision, OEM ID, OEM table ID, OEM revision, creator, creator revision and
  checksum — the set Linux prints at boot, readable straight against `dmesg`,
  with no flag. The fields were split across a default row and a `-v` row until
  the split stopped paying: the two that answer real questions on real firmware
  (CREATOR, and ADDRESS) were both behind the flag. 75 columns of data under a
  76-column header, so it still fits an 80-column console. `-v` now means one
  thing — *also* dump the raw bytes for a table that already decoded — and is
  refused on the inventory rather than silently ignored.

- **`axl update`, `use`, `list`, `uninstall`** — the install maintains itself.
  `install.sh` now ships *inside* the prefix (`libexec/axl/`), so removing or
  updating an SDK no longer requires re-fetching the script that placed it:
  the install knows its own version, prefix and links, and a script on a web
  server knows none of them. Borrowed from Qt Installer Framework's
  MaintenanceTool, which installs its updater alongside the product. It stages
  at mode 0644 deliberately — `axl` lists the *executables* in `libexec/axl/`
  as commands, so the mode bit is what keeps it from also appearing as a
  third spelling, `axl install`. A prefix that was not placed by `install.sh`
  (a distro package, or a source-tree stage) says so and points at the right
  tool instead. `axl update` upgrades in place — a new versioned root beside
  the old one, the `current` symlink moved, the previous version kept for
  rollback and bounded by `axl prune`. Running it when already current costs
  one small request and changes nothing rather than re-extracting the same
  tree. `axl use <version>` makes another version current — instant and
  offline when it is already on disk, downloading only when it is not — and
  `axl list` shows what is installed with `*` on the current one. `update`
  deliberately refuses `--version`: moving backwards is `use`, not update.
  A selection verb was overdue rather than new scope: `axl prune` already
  keeps current + one previous so a rollback target exists, and nothing could
  activate it.

- **The host-tools tarball ships `bin/axl` and uses the SDK's layout.** It was
  a flat `scripts/` directory with no dispatcher, which made it the one channel
  whose users could not run `axl <cmd>` and were therefore forced to hardcode
  `<root>/scripts/...` — the exact thing the dispatcher exists to stop. It is
  now `bin/axl` + `libexec/axl/` + `share/axl/version`, the same shape the SDK
  tarball and the package use, so `axl` resolves its own prefix wherever the
  archive is extracted. Four files that no host tool references are gone with
  it: `pe-set-debug.c`, two `.lds` linker scripts and `uefi-manifest.json5` —
  build inputs in a bundle that carries no compiler.

- **`install.sh` creates the `current` symlink `axl prune` has always
  protected.** `<root>/axl-sdk -> axl-sdk-<version>` was documented in
  `AXL-Distribution-Design.md` §12.2 and honoured by `axl-prune.sh`, but
  nothing ever created it — so that protection was unreachable unless a user
  made the link by hand, and consumers had no stable path to pin. Guarded to a
  versioned root: a checkout's default `./stage` prefix must not start dropping
  symlinks into the parent of the source tree, so only a prefix that is itself
  `axl-sdk-<version>` gets one. The target is relative, so the root stays
  relocatable.

- **The public source snapshot stops publishing internal working notes, and
  gets a gate (§15).** Every release pushes a squashed source snapshot to the
  public `aximcode/axl-sdk-releases`. The selection rule was "everything
  tracked, minus `.github/`" — one exclusion, for a mechanical reason rather
  than an editorial one — so 16 session handoffs, 37 plans and specs under
  `docs/superpowers/`, and an infra runbook that diagrams a reverse-tunnel
  topology were all public. Nobody decided that; they were public because the
  rule had no opinion. Now excluded **by class**, so a new file joins the right
  side by where it is put.

  `scripts/make-source-snapshot.sh` assembles and prunes;
  `scripts/check-snapshot-clean.py` gates the **assembled** tree rather than
  the repo, so an exclusion that silently stops working still fails.

  **It found a leak the design doc's own inventory had missed.** That inventory
  named two files; a third, `docs/ROADMAP-Archive.md`, carried two real lab
  IPs, a service tag and an iDRAC credential line — and no exclusion class
  covered it, so excluding working notes would not have helped. Redacted at
  source.

  Two things about its shape are deliberate. **The committed rules name
  nothing**: a tracked file listing the hostnames it forbids would publish them
  on the next snapshot, so the committed patterns are structural (private and
  CGNAT ranges, ssh tunnel directives) and the specific strings arrive as a CI
  secret — which the test asserts is never echoed into the output, because a
  public build log would publish it just as well. And **prose is scanned
  strictly while fixtures and vendored code are not**: a private address in
  mbedTLS's x509 test data or in `axl_ipv4_parse_cidr()`'s docstring is an
  input or an example, and the first draft flagged 64 of those; a private
  address in prose is where a real machine gets written down.

  `test-source-snapshot.sh` plants each forbidden class and requires the gate
  to fire, requires a missing or near-empty snapshot to be **refused** rather
  than reported clean, and requires the same address in a test fixture *not* to
  fire — a gate that cries wolf over test data gets switched off.

- **Consumer-side verification — the first checks that look at what a consumer
  actually receives (§16).** Everything else in the tree runs *before*
  publication, on locally built files, on a builder that already has the cross
  toolchain and the mbedTLS submodule.

  `test/integration/test-consumer-install.sh` is the gate: on clean
  `debian:stable-slim` and `fedora:latest` with no compiler, linker or
  archiver, it `curl`s `install.sh` out of a release directory and runs it,
  then compiles a real PE32+ `.efi`, runs every dispatcher command, and
  exercises `axl list` / `axl uninstall` / `--host-tools`. §16 asked for those
  two distributions because one takes `.deb` and the other `.rpm`; with the
  packages gone the pair is kept for a better reason — **Debian's `/bin/sh` is
  dash and Fedora's is bash**, and `install.sh` is `#!/bin/sh`, so a bashism is
  invisible on one and fatal on the other.

  `scripts/check-published-release.sh` is the post-publish half, run from
  `release.yml` after the release is out. It carries **no list of expected
  asset names** — that would be a fifth copy of the one format string — and
  instead cross-checks the release against itself: everything `SHA256SUMS`
  lists is attached, everything attached is checksummed, every asset downloads
  and matches, `VERSION` agrees with the tag, and the two stable
  `latest/download/` URLs resolve. A rename that missed one place is a set
  difference in whichever direction it went. Being version-agnostic it works on
  older releases too — verified live against v4.4.0. Its own test injects each
  failure it exists for and requires it to be named.

- **`make check-asset-names`** — the release asset names, held equal across the
  places that spell them and cannot share code. `packaging/install.sh` is why
  they cannot: it runs on a machine with nothing of ours installed, so it
  constructs the names itself, and a rename on either side is invisible until a
  user's `axl update` cannot find its download. The gate compares **computed**
  names, not spellings — it asks the two producer scripts (`--print-name`) and
  runs `install.sh`'s own `asset_candidates()` — so a renamed variable or an
  added indirection cannot fool it. It also requires each component to keep a
  legacy fallback.

### Fixed

- **Rolling back an SDK version stranded the installation.** `install.sh`
  symlinks every executable in `<prefix>/bin` into your bin directory — and
  `axl` is one of them, so switching versions switched the **manager** too.
  Roll back to a release predating the staged installer and both `axl update`
  and `axl use` died with *"no installer at …"*; the only escape was
  re-fetching `install.sh` from a web page, which is precisely the dependency
  the staged copy exists to remove. `axl prune` deliberately keeps a previous
  version so that rollback is possible, so the retention policy shipped
  together with the trap it walked into.

  `axl` is now pinned to the newest installed prefix carrying a staged
  installer, whichever version is current — re-resolved on every install
  rather than "never downgrade", which would dangle the moment that newer
  prefix was uninstalled. Only `axl` is treated this way: `axl-cc`, `axl-c++`
  and `axl-install-toolchain` are per-version tools and must follow the active
  SDK, which is the point of switching.

  And because anyone stranded *before* this fix cannot run `install.sh` to get
  it — fetching it is the thing they lost — `axl` also looks sideways itself,
  taking the newest sibling prefix's installer. `AXL-Distribution-Design.md`
  §20.

- **`axl prune` told a non-versioned prefix to "upgrade with apt/dnf".** That
  advice has been wrong since the packages retired, and it was the message a
  user got whenever something other than `install.sh` had placed the tree. It
  now says what is actually true: something else owns this install, so pruning
  beside it could delete files this script does not own. Found while checking
  whether an externally-managed layout is safe for our own verbs — see the
  asdf/mise note in `AXL-Distribution-Design.md` §18.3.

- **`install.sh` never mentioned `python3`, and four shipped commands need
  it.** `axl rsod-decode`, `axl gdb-syms`, `axl extract-fv-shell` and
  `axl emulate` are Python; neither `debian:stable-slim` nor `fedora:latest`
  ships an interpreter. A user following `axl-cc`'s own crash diagnostics to
  `rsod-decode` got `python3: command not found` with nothing pointing at the
  cause. It now **advises and does not refuse** — the same shape as the QEMU
  probe — because `axl-cc` and `axl-c++` need no interpreter and a user who
  only wants to build a `.efi` must not be blocked. Found by the new
  containerized consumer gate on its first run, which is the class of defect
  §16.1 predicted: "nothing starts from a clean machine."

- **`install.sh` refused a mirror's regenerated `SHA256SUMS`.** `find . -exec
  sha256sum {} +` and `sha256sum ./*.tar.gz` both write `./name` entries, which
  `sha256sum -c` accepts. The asset lookup did not, so a `--base-url` mirror
  got "release X publishes none of: …" about a directory that plainly held the
  asset. The prefix is now stripped before comparing.

- **CI had been red for at least three commits on a test that passes locally.**
  `test-release-watch-scope.sh` drove `watch-release-runs.sh` with the tag
  `v4.3.1` and resolved it in the *ambient* repo. `actions/checkout` is shallow
  and fetches **no tags**, so `git rev-parse v4.3.1^{commit}` failed, the
  watcher exited 2 before polling anything, and six of eight assertions failed
  — on every push, indistinguishable from a genuine break.

  The two that still "passed" are the more interesting half: both assert
  `rc != 0`, which a script that dies on startup satisfies perfectly. A control
  a crash can satisfy is not a control. The test now runs the watcher inside a
  throwaway git repo carrying its own **annotated** `v4.3.1` (annotated
  deliberately — `git rev-parse` on one yields the *tag object's* sha, so it
  exercises the `^{commit}` peel that the watcher's own comment records as a
  real bug), and every negative assertion additionally requires the watcher to
  have reached its `Watching …` line. Sabotaging the fixture tag now scores
  **0 passed, 12 failed** where it used to score 2 passed, 6 failed.

  Verified both ways: 8/8 in a normal clone, and 8/8 in a `--no-tags --depth 1`
  clone, which is exactly the shape CI checks out.

- **17 shipped tools were missing from `README.md`'s tool table**, `lsacpi`
  among them — absent since it shipped in 4.3.3. That table is the public
  front page: `aximcode/axl-sdk-releases` serves a byte-identical README, so
  the omission was visible in both places. Fourteen rows added (`ata`,
  `axbench`, `fwtool`, `i2c`, `kbtune`, `lsacpi`, `mkfixture`, `netload`,
  `nvme`, `rndisfix`, `scsi`, `smart`, `tar`, `timetest`); `crashtest`,
  `fbcon` and `kbtune-drv` stay out as a fault fixture, a console app and a
  driver. `make check-tool-docs` now fails on a shipped tool with no row, a
  row for a tool that no longer ships, or two rows for one tool — it does NOT
  generate the table, because the rows are hand-written prose up to ~320
  characters and the only machine-readable descriptions available are
  ~40-character console labels. Completeness is what is checkable; content is
  not.
- **The host-tools tarball shipped four Python tools that could not start.**
  Adding `axl_version.py` for `--version` put it in `install.sh`'s staging list
  and not in `release.yml`'s, so `rsod-decode`, `gdb-syms`, `extract-fv-shell`
  and `axl-emulate` would all have died with `ModuleNotFoundError` in the next
  release. The tarball's smoke test could not see it: it asserted
  `test -x rsod-decode.py`, which stays true of a script that dies on import.
  The set now has ONE owner — `HOST_TOOL_FILES` in the Makefile — which both
  `install.sh` and `release.yml` read, and `make check-tool-version` fails if
  either grows its own copy again. The smoke test now RUNS every command in the
  extracted tarball and checks it reports the release version.
- **`axl prune` pruned SDK roots it had no business scanning.** On a package
  install the prefix is `/usr`, so the root resolved to `/` — and any
  `axl-sdk-<version>` directory sitting there was a candidate. It now runs the
  SDK half only when the running prefix is itself a versioned root, and says
  "not applicable" otherwise instead of printing `root: /` and leaving the user
  to reason about whether an anchored pattern makes that safe. Toolchain
  pruning is unchanged and still runs in both layouts.
- **`lsacpi -v`'s column headings sat one place right of their own data.**
  One extra space in the header literal shifted `OEM REV`, `CREATOR`,
  `CREATOR-RV` and `CHECKSUM` off their columns. The default row was always
  correct; `-v` had no test, which is how it shipped.
- **The FACS row was missing `address` in `--json`**, alone among the tables,
  even though the text `-v` row printed one for it. Unlike revision and the
  OEM strings — genuinely absent from FACS, whose offsets hold other data —
  the address is the pointer the tool already holds.
- **A zeroed creator ID read two ways in one JSON document**: `null` for
  FACS's absent fields but `""` for QEMU's BGRT. It is `null` in both now.
- **`lsacpi <SIG> -v` reported less than the inventory did** — no address, no
  OEM table ID, no OEM revision and, despite being the field the `CREATOR`
  column was added for, no creator.

### Changed

- **The installer no longer suggests a target architecture.** `--toolchain`
  selects the **firmware you are building for**, which the host cannot infer,
  and a wrong guess is a 239 MB (x64) or 500 MB (aa64) download. It has always
  defaulted to installing none; what changed is the documentation, which
  presented `--toolchain x64` as though it were the norm — an x86-64-centric
  example that would have had to be unpicked for a non-x86-64 host later.
  `axl-install-toolchain x64|aa64|all` is now shown as the deliberate second
  step, with the one-command form as the shortcut it is.

- **The UEFI-tools tarball's own README listed 13 of 38 tools; it is now
  derived.** `scripts/make-uefi-tools-readme.py` builds the list from
  `TOOL_NAMES` plus `devkit.conf`'s `desc:` lines — which `check-devkit-conf`
  already requires one of per tool, so the text has an owner with a gate on
  it. §14.3 asked for both prose tool lists to be derived; they went opposite
  ways on purpose. `README.md`'s rows are hand-written prose up to ~320
  characters about real flags, against `devkit.conf`'s ~40-character console
  labels, so deriving it would swap documentation for labels — completeness is
  what is checkable there, and `make check-tool-docs` checks it. The tarball's
  entries *are* labels, so that one derives.

  It was also an inline heredoc in `release.yml`, so nothing could look at it
  without cutting a release. Moving it out mattered for a second reason: those
  92 lines carry mbedTLS's Apache-2.0 election, EDK2's BSD-2-Clause-Patent
  notice, iPXE's GPL-2.0-or-later notice and a **GPL-2.0 §3(b) written
  offer** — obligations, not prose — so
  `test/integration/test-uefi-tools-readme.sh` asserts each survives
  regeneration, and `check-tool-docs` asserts the workflow still delegates to
  the generator so the heredoc cannot grow back.

  Descriptions are wrapped to 78 columns with a hanging indent and folded to
  ASCII: the file ships on a FAT stick beside the `.efi` tools and is read
  with `cat README.txt` in the UEFI Shell, which is 80 columns and draws a
  block for the em dashes `devkit.conf` contains.

- **The install documentation is one `curl`, and the C++ section is no longer
  wrong.** README's install prose is 185 lines against 243, with a single
  `curl` on the install path against eight; `getting-started.rst` stops
  mirroring it and links to README for pinning, mirrors and the
  host-tools-only install. `### Pinning, mirrors and air-gapped installs` and
  `### Building axl-sdk from source` became their own sections — both had been
  sitting *inside* `### Using your own toolchain`, where nobody would look for
  them.

  Length was the smaller half. `getting-started.rst` opened by telling readers
  to `apt install gcc binutils gcc-aarch64-linux-gnu` — the exact advice
  README's own Requirements section says is obsolete — and its C++ section
  listed exceptions, RTTI, `<string>`, `<vector>`, `<unordered_map>`,
  `<stdexcept>`, `thread_local` and `<format>` as **forbidden**, "require
  libsupc++/libstdc++ symbols we don't have". None of that has been true since
  P4 gave every C++ link the toolchain's own libstdc++. Each was compiled and
  linked to check: containers on both arches with no flag, `throw`/`catch`
  under `-fexceptions`, `typeid`/`dynamic_cast` under `-frtti`. What survives
  is the *cost* — exceptions measured +28-36% of `.efi` on four real x64 tools
  — which is why the defaults did not move with the capability.

  `AXLMM-Design.md` repeats the same list and is published on the docs site,
  so it gained correction banners at the two claims a reader would act on,
  following the convention it already used for `libaxl-cxx.a`. `--hosted` was
  described in three places as a "warned no-op"; it is an error (exit 1),
  because the thing it switched off no longer exists.

- **The release publishes `install.sh` and `VERSION`, every asset carries its
  version, and the `.deb`/`.rpm` are retired.** One format string now names
  everything — `axl-sdk-<component>-<ver>[-<arch>].tar.gz` — so the assets are
  `axl-sdk-linux-<ver>-x86_64.tar.gz`, `axl-sdk-host-tools-<ver>.tar.gz` and
  `axl-sdk-uefi-tools-<ver>-<arch>.tar.gz`, beside the three stable-named
  `install.sh`, `VERSION` and `SHA256SUMS`.

  Only two names needed to stay stable, and that is what freed the rest:
  `install.sh` is the front door and `VERSION` is what it resolves `latest`
  against. Everything else is fetched by a script that already knows the
  version and uses the versioned tag URL. Six of seven peer projects that ship
  packages from GitHub releases version their filenames; the reason we could
  not was nine hand-typed `latest/download` commands, which the installer
  replaces.

  `axl-sdk-tools-<arch>` → `axl-sdk-uefi-tools-<ver>-<arch>` is the
  load-bearing rename: it is the one asset whose contents do **not** run on the
  machine that downloaded them, and the old name said the opposite. `<arch>`
  now means the host on the SDK tarball and the target firmware on the
  uefi-tools ones; host-tools carries none, because its payload is shell and
  Python.

- **`install.sh` picks the asset out of `SHA256SUMS` instead of guessing, and
  still installs releases published before the rename.** It downloads
  `SHA256SUMS` first — stable-named across every release ever cut, and the
  authoritative list of what that release contains — then takes the first name
  it recognises. So `axl use 4.4.0` still works: the fallbacks are
  `axl-sdk-<ver>-linux-x86_64.tar.gz` and the unversioned
  `axl-sdk-host-tools.tar.gz`. Without them the rename would have broken
  rollback for every existing release on the day it shipped — and rollback is
  the entire reason `axl prune` retains current-plus-one.

  Reading the sums first also means a 13 MB download is no longer spent before
  learning there is nothing to verify it against, and that a missing asset is
  distinguishable from an unreachable mirror: both are the same failed `curl`,
  but only one appears in a `SHA256SUMS` we successfully fetched.

- **Both remaining tarbombs are fixed, and `install.sh` is safe against the
  ones already published.** `axl-sdk-host-tools` unpacked six entries into the
  caller's working directory and `axl-sdk-tools-<arch>` unpacked forty-three;
  each now extracts to a single versioned root matching its own name. The
  installer extracts into a staging directory *inside* the prefix root and
  renames it into place, so a legacy tarbomb gets the root it should have had
  instead of scattering beside the versioned trees `axl prune` walks — and the
  final move is a same-filesystem rename, so an install is there or it is not,
  never half-extracted.

- **The SDK tarball carries the licence payload the packages used to.** The
  `.deb`/`.rpm` staged 75 doc files and the tarball staged none, which was
  survivable only while the packages existed. It now ships `LICENSE`, `NOTICE`,
  `THIRD_PARTY.md`, `CHANGELOG.md`, `README.md`, the examples as source, and
  the mbedTLS / DejaVu / libvterm / FreeType / edk2 licence texts. Four of
  those five are obligations that attach to redistributing `libaxl.a` rather
  than documentation, so losing them with the packages would not have been a
  cosmetic regression.

- **The host-tools tarball has a local reproduction.**
  `scripts/make-host-tools-tarball.sh` replaces ~90 lines assembled inline in
  `release.yml` — the defect `make-sdk-tarball.sh`'s own header had named — so
  the release and `test/integration/test-host-tools-tarball.sh` run the same
  code, and its name, layout and every command it ships are now checkable on a
  laptop instead of only by cutting a tag.

- **`test-pkg-deps-minimal.sh` → `test-host-deps-minimal.sh`, sourced from
  `install.sh`.** It read the required host commands out of the `.deb`'s
  `--depends` metadata; with the packages gone the declaration moved to
  `packaging/install.sh`'s own `need_cmd` calls, which is a better owner
  because the installer *enforces* it at run time rather than asserting it at
  build time. A `need_cmd` with no Debian package mapping is now a hard
  failure, so adding one forces a decision about how a minimal image gets it.

- **`axl_tls_available()`, `axl_jose_available()` and `axl_pk_available()`
  are documented as guaranteed trues.** mbedTLS has been an unconditional
  dependency since the `AXL_TLS` toggle was removed, and no `#ifdef
  AXL_HAVE_TLS` remains in the tree, so these cannot answer no. The dead
  guards written against them are gone, and the docs no longer suggest
  writing one: a fail-closed branch on a predicate that cannot fail is
  decoration, not a security property. They remain exported so existing
  consumer source keeps compiling. The predicate that *can* still answer no
  is `axl_pk_alg_available()`, which reports whether this image linked a
  given algorithm — `crypto.rst` and two `axl-crypto.h` docstrings had been
  pointing at `axl_pk_available()` for that, which it never did.
