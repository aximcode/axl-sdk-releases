...
function(_axl_build_efi TARGET TYPE)
    ...
    #
    # The LINK step deliberately gets no OPTIONS. axl-cc derives what the link
    # needs (the C++ runtime archive, -frtti's libstdc++, the exceptions linker
    # script and glue) from the OBJECTS via `nm -u`, precisely so a staged
    # build works -- so passing the compile flags again would be noise, and
    # forgetting to would be a silent defect. This is that design paying off.
