    ...
    )
{
    ...
    /* Decode LoadOptions if present AND the AXL magic header matches.
     * SERVICE_LO_MAGIC distinguishes payloads that
     * axl_service_start_embedded produced from UCS-2 LoadOptions a
     * shell user might pass — the latter would decode as garbage. On a
     * mismatch (or no LoadOptions at all), defaults from the
     * descriptor table apply. */
    const void *lo_buf  = NULL;
    size_t      lo_size = 0;
    if (svc->opts_descs != NULL && svc->user != NULL) {
        m_drv_cfg = axl_config_new(svc->opts_descs, NULL, svc->user);
        /* Defaults already auto-applied by axl_config_new. Now layer on
         * the launcher's serialized values, if any. */
        if (m_drv_cfg != NULL
            && axl_driver_get_load_options_raw(&lo_buf, &lo_size) == AXL_OK
            && lo_size > SERVICE_LO_MAGIC_LEN
            && axl_memcmp(lo_buf, SERVICE_LO_MAGIC,
                          SERVICE_LO_MAGIC_LEN) == 0) {
            const char *lo_body = (const char *)lo_buf + SERVICE_LO_MAGIC_LEN;
            if (axl_config_from_string(m_drv_cfg, lo_body) != AXL_OK) {
