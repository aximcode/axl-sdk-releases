...
sentinel. `axl_pci_get_class_code`, `axl_pci_get_header_type` and
`axl_pci_get_subsystem` share the same absent-function check
internally — their `-1` return covers "function not present"
alongside the type-specific rejection cases, and the out parameter
is left untouched. `class_code` matches the shape consumed by
`axl_pci_find_by_class`.

**`axl_pci_get_class_code` changed after 4.2.0.** Up to and including
that release it was the one accessor with no presence precheck,
returning `AXL_OK` with `0xFFFFFF` for an absent function rather than
-1. Code that treats a non-`AXL_OK` return as a *bus error* worth
reporting will now see absent functions as well.

Note the width while you are here — the sentinel is the 24-bit
`0xFFFFFF`, and comparing against a 32-bit `0xFFFFFFFF` is a skip that
silently never fires. `0xFFFFFF` is still reachable on success: it is a
legitimate reading for a *present* but class-less function (base class
0xFF), so it means "no usable class", never "nothing there".

The precheck reads the vendor ID at offset 0x00 and tests it against
`0xFFFF`. Firmware that answers an absent function with some other
constant defeats it — as it defeats the sibling accessors — so a
consumer on such a platform still needs its own filter.
